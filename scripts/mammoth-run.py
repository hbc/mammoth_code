...
def _get_mammoth_genomic(chrom, pos_hit, m, genome):
    pos = pos_hit + m
    a = pybedtools.BedTool("{0}\t{1}\t{2}".format(chrom, pos-150, pos+150), from_string=True)
    ...
    post = seq[1][150:]
    # print [pre, nt, pos]
    return "%s\t%s\t%s-%s-%s" % (chrom, pos, pre, nt, post)
...
def _format(info, exons, flank_m):
    ...
    return "%s %s %s %s %s %s %s %s %s %s" % (info[0], info[1], info[2], info[5], changes, exons['chrom'], genomic_pos, qc, flank, flank_m)
                ...
                for e in matches[m][t]['changes']:
                    if "changes" in matches[m][t]['changes'][e]:
                        for p in matches[m][t]['changes'][e]['changes']['positions']:
                            ...
                            print >>outh, _format([m, t, e, info, ratio, en], exons[e], flank_m)
